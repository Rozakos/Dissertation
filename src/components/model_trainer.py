...
class ModelTrainer:
    ...
    def initiate_model_trainer(self, train_array, test_array):
        try:
            ...
            # Defining models and their respective hyperparameters
            models = {
            ...
            # Evaluating models with the defined parameters
            model_report = evaluate_models(X_train=X_train, y_train=y_train, X_test=X_test, y_test=y_test, models=models, param=params)

            # Selecting the best model based on the evaluation
            best_model_score = max(model_report.values())
            best_model_name = max(model_report, key=model_report.get)
            best_model = models[best_model_name]

            # Check if the best model's score is acceptable
            if best_model_score < 0.6:
                raise CustomException("No suitable model found with adequate performance.")
            ...
            # Making predictions with the best model and calculating R2 score
